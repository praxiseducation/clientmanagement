      ...
      <body>
        ...
        <script>
          ...
          function preloadRecapData() {
            console.log('preloadRecapData called on mouseover');
            ...
            if (typeof PerformanceCache === 'undefined') {
              console.log('PerformanceCache not yet defined, skipping recap preload');
              return;
            }
            ...
            if (cachedData) {
              console.log('Recap data found in cache for mouseover preload');
              return; // Data is already cached
            }
            
            console.log('Preloading recap data for:', clientInfo.name);
            ...
            google.script.run
              .withSuccessHandler(function(clientData) {
                if (clientData) {
                  ...
                  google.script.run
                    .withFailureHandler(function(error) {
                      console.log('Failed to cache recap data server-side:', error.message);
                    })
                    .cacheRecapData(clientData);
                    
                  console.log('Recap data preloaded from server and cached');
                }
              })
              .withFailureHandler(function(error) {
                console.log('Background recap data preload failed (non-critical):', error.message);
              })
...
function getClientDataForRecap() {
  ...
  // Also get quick notes if they exist
  const scriptProperties = PropertiesService.getScriptProperties();
  const notesKey = `notes_${sheetName}`;
...
/**
 * Cache recap data in script properties for fast server-side access
 */
function cacheRecapData(clientData) {
  ...
  try {
    ...
    scriptProperties.setProperty(cacheKey, cacheData);
    console.log('Recap data cached in script properties for:', clientData.studentName);
  } catch (error) {
...
/**
 * Get complete client data for Preview Update dialog using most efficient method
 * Uses Enhanced Cache first, then fallback to Script Properties + Sheet access
 */
function getCompleteClientDataForPreview(sheetName) {
  // Method 1: Try Enhanced Client Cache first (fastest)
  try {
    const cachedClient = EnhancedClientCache.getClientBySheetName(sheetName);
    if (cachedClient && cachedClient.studentName) {
      console.log('Retrieved client data from Enhanced Cache:', cachedClient.studentName);
      return {
        studentName: cachedClient.studentName,
        studentFirstName: (cachedClient.studentName || '').split(' ')[0],
        parentName: cachedClient.emailAddressees || 'Parents',
        parentEmail: cachedClient.parentEmail || '',
        clientTypes: cachedClient.clientTypes || '',
        clientType: cachedClient.clientTypes || '',
        ...
        isActive: cachedClient.isActive !== undefined ? cachedClient.isActive : true
      };
    }
  } catch (e) {
    console.log('Enhanced Cache not available, falling back to Script Properties');
  }
  
  // Method 2: Use same pattern as successful Session Recap dialog
  const scriptProperties = PropertiesService.getScriptProperties();
  ...
  // Read core data from sheet (same as getClientDataFromSheet)
  const studentName = sheet.getRange('A1').getValue() || sheetName.replace(/([A-Z])/g, ' $1').trim();
  const clientTypes = sheet.getRange('C2').getValue() || '';
  
  // Get cached data from Script Properties (faster than sheet access)
  ...
  console.log('Retrieved client data from Script Properties + Sheet:', studentName);
  
  return {
    studentName: studentName,
    studentFirstName: (studentName || '').split(' ')[0],
    parentName: emailAddressees || 'Parents',
...
function processRecapPreview(formData) {
  try {
    ...
    console.log('Retrieved complete client data:', clientData);
    
    // Get both client data and quick notes from cache for better performance
    const scriptProperties = PropertiesService.getScriptProperties();
    ...
    // Save the extended notes back to cache
    scriptProperties.setProperty(notesKey, JSON.stringify(extendedNotes));
    scriptProperties.setProperty(`notes_timestamp_${sheetName}`, new Date().getTime().toString());
    ...
    const previewData = {
      ...
      subject: `${(clientData.studentName || '').split(' ')[0] || 'Student'} Session Update - ${new Date().toLocaleDateString()}`
    };
    
    console.log('=== PROCESS RECAP PREVIEW - SIMPLIFIED APPROACH ===');
    console.log('Using client data:', clientData.studentName);
    console.log('Passing form data with sheetName:', previewData.formData.sheetName);
    console.log('Subject:', previewData.subject);
    console.log('showUpdatePreviewDialog will retrieve fresh complete client data');
    console.log('=== END PREP DEBUG ===');
...
function sendIndividualRecap() {
  ...
  const cachedData = scriptProperties.getProperty(cachedDataKey);
  
  if (cachedData) {
    try {
      clientData = JSON.parse(cachedData);
      console.log('Using cached recap data for faster loading');
    } catch (e) {
...
function getSessionRecapJavaScript() {
  return `
    ...
    document.addEventListener('DOMContentLoaded', () => {
      console.log('Session recap dialog loaded, fetching data...');
      ...
      google.script.run
        .withSuccessHandler(function(data) {
          console.log('Received recap data:', data);
          recapData = data;
          populateDialog(data);
    ...
    function populateDialog(data) {
      console.log('Populating dialog with:', data);
      ...
      console.log('Dialog populated successfully');
    }
    ...
    function addHomework() {
      ...
      if (select && textarea && select.value) {
        console.log('Selected template:', select.value);
        ...
        if (select.value === 'ACT Practice Test') {
          console.log('Showing ACT test selector');
          ...
          console.log('Adding template to textarea:', select.value);
          textarea.value = currentValue + select.value;
          select.selectedIndex = 0;
        }
      } else {
        console.log('No template selected or elements not found');
      }
    }
    
    // Handle ACT test selection
    document.getElementById('actTestDropdown').addEventListener('change', function() {
      console.log('ACT test dropdown changed:', this.value);
      if (this.value) {
        const textarea = document.getElementById('homework');
        let currentValue = textarea.value;
        if (currentValue && !currentValue.endsWith('\\n')) {
        ...
        console.log('Added ACT test to homework:', this.value);
    ...
    document.getElementById('recapForm').addEventListener('submit', function(e) {
      ...
      google.script.run
        ...
        .withFailureHandler(function(error) {
          console.error('Failed to process recap:', error);
          if (loadingOverlay) {
            loadingOverlay.classList.remove('active');
          }
          alert('Failed to process recap: ' + error.message);
        })
        .showUpdatePreviewDialog({formData: formData});
    });
  `;
}
...
function showUpdatePreviewDialog(emailData) {
  ...
  console.log('showUpdatePreviewDialog: Using complete client data for:', completeClientData.studentName);
  
  // Store the merged data in Properties Service
  const userProperties = PropertiesService.getUserProperties();
  userProperties.setProperty('tempEmailData', JSON.stringify(mergedEmailData));
  console.log('Stored complete merged data in tempEmailData');
      ...
      <body>
        ...
        <script>
          ...
          function initializeDialog() {
            ...
            console.log('=== PREVIEW UPDATE DIALOG DATA MAPPING DEBUG ===');
            console.log('Full emailData object:', emailData);
            console.log('emailData.subject:', emailData.subject);
            console.log('emailData.formData:', emailData.formData);
            console.log('Key fields check:');
            console.log('  - studentName:', emailData.formData.studentName);
            console.log('  - studentFirstName:', emailData.formData.studentFirstName);
            console.log('  - parentName:', emailData.formData.parentName);
            console.log('  - parentEmail:', emailData.formData.parentEmail);
            console.log('  - clientTypes:', emailData.formData.clientTypes);
            console.log('  - dashboardLink:', emailData.formData.dashboardLink);
            console.log('  - tutorName:', emailData.formData.tutorName);
            console.log('Recap-specific fields:');
            console.log('  - focusSkill:', emailData.formData.focusSkill);
            console.log('  - todaysWin:', emailData.formData.todaysWin);
            console.log('  - homework:', emailData.formData.homework);
            console.log('  - nextSession:', emailData.formData.nextSession);
            console.log('  - practiced:', emailData.formData.practiced);
            console.log('  - introduced:', emailData.formData.introduced);
            console.log('  - mastered:', emailData.formData.mastered);
            console.log('Dialog-specific field mappings:');
            console.log('  - openingNotes:', emailData.formData.openingNotes);
            console.log('  - struggles:', emailData.formData.struggles);
            console.log('  - parentNotes:', emailData.formData.parentNotes);
            console.log('  - progressNotes:', emailData.formData.progressNotes);
            console.log('Meeting Notes Link at top level:', emailData.meetingNotesLink);
            console.log('Meeting Notes Link in formData:', emailData.formData.meetingNotesLink);
          ...
          function openMeetingNotes() {
            console.log('openMeetingNotes called');
            console.log('emailData:', emailData);
            console.log('meetingNotesLink:', emailData ? emailData.meetingNotesLink : 'emailData is null');
            ...
            if (emailData && emailData.meetingNotesLink) {
              console.log('Opening meeting notes link:', emailData.meetingNotesLink);
